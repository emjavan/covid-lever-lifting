#############################################################################################
## Code for simulations that exit after a certain number of days
#############################################################################################
...
#' @param total_days the number of days to run simulation
#' @param init_infected the initial number of undetected infected individuals to begin simulation

#' @return A list of length num_reps, where each component is a single call to run_covid_sim_time.
covid_params_fn = function(base_r_not           = 1.5,
                           ...
                           total_days 		      = 60,
                           init_infected        = 100,
                           post_lever_lift_increase = 0.5,
                           post_lever_lift_r_not = base_r_not + post_lever_lift_increase
...
run_n_covid_sims <- function(num_reps, ...) {
  plyr::rlply(.n = num_reps, .expr = run_covid_sim_time(...) )
}
...
run_covid_sim_time <- function(params) {
  with(params,{
    ...
    ## Initialize the epidemic with undetected infectious
    UI_Intro_Symp[1] = init_infected
    ...
    time_record = data.frame(matrix(data = 0, ncol = 13))
    colnames(time_record) = c("Day_Count", "New_Exposed", "New_Infection", "Total_Infections", "Cumulative_Infections",
                              "New_Intro", "Total_Intro_Infections", "Cumulative_Intro_Infections",
                              "New_Detections", "Cum_Detections", "New_Intro_Detections", "Cum_Intro_Detections", "R0")
    
    ## Initialize appropriately
    time_record$R0 = base_r_not
    time_record$New_Infection = time_record$Total_Infections = time_record$Cumulative_Infections = I
    time_record$Day_Count=1
    day_count=1
    lever_lift_count=0
    max_rebound = init_infected*10
    #while( (day_count < total_days) ){
    while  (( I < max_rebound) & ((I+sum(incubationInfecteds)) > 0)) {
      ...
      #Update the day count, by end of while loop one more day has passed; count increase must go before rbind
      day_count = day_count + 1
      
      #adding time step data
      time_record <- rbind(time_record, c(day_count, newExposed, newInf, I, cumI, newInf_Intro, I_Intro, cumI_Intro, newlyDisc, cumD, newlyDisc_Intro, cumD_Intro, base_r_not))
    
      # get the maximum number of detected cases over the last 14 days
      if(day_count < 14){
        max_det_in_one_day = max(time_record$New_Detections[1:day_count])
      }else{
        max_det_in_one_day = max(time_record$New_Detections[(day_count-13):day_count])
      }
      
      # if the number of detections is equal to max increase counter, else decrease
      if(time_record$New_Detections[day_count] == max_det_in_one_day  & max_det_in_one_day != 0){
        lever_lift_count = lever_lift_count + 1
      }else{
        lever_lift_count = lever_lift_count - 1
      }
      
      # once sufficiently low counter and level lifting is true increase R0
      if(lever_lift_count <= -14){
      #if(lever_lift_count <= -28){
        base_r_not = post_lever_lift_r_not
        prop_p = base_dispersion/(base_r_not+base_dispersion)
      }
    } # end while loop
    return(time_record)
  }) # end with params
} # end function run_covid_sim_time
...
save_covid_runs <- function(r_not,
  ...
  parms <- covid_params_fn(base_r_not = r_not, 
                           init_infected = init_num_infected,
                           post_lever_lift_increase = increase_r_not,
                           ...)
  
  saved_file_path <- get_save_path(r_not, init_num_infected, increase_r_not, num_reps)
